...
<p>Spreading preview codes for pages marked 'published' is ill-advised: whenever a preview authentication code is spread for a page, the lifetime of said authCode equals the the time until that page, or its attributes, are altered. We may assume that an already 'published' page is quite stable, hence implicitly produces a longlived authCode. Keep in mind that an authCode is <em>site-wide</em>, so a longlived authCode means <em>any</em> part of the (then hidden/unpublished) site is visible to anyone having obtained such an authCode (through any means) for a very long time!<br /> We counter this by doing two things:</p>
