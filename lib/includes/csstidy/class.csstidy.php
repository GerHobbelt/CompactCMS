...
/* [i_a] wasn't set, so failed to set through set_cfg() due to checks in there */	
...
function load_template($content, $from_file=true)
{
	...
	if($content === 'high_compression' || $content === 'default' || $content === 'highest_compression' || $content === 'low_compression')
	{
		...
		/* [i_a] update the 'template' setting as well */
		if($content === 'default')
			$this->settings['template'] = 'default';
		elseif($content === 'highest_compression')
			$this->settings['template'] = 'highest';
		elseif($content === 'high_compression')
			$this->settings['template'] = 'high';
		else
			$this->settings['template'] = 'low';
		return;
	}
    ...
    for ($i = 0, $size = strlen($string); $i < $size; $i++ )
    {
        ...
        case 'ip':
            if(csstidy::is_token($string,$i))
            {
                ...
                {                       
                    /* IE hack fix; git commit 519a0f768571dbcdb31c */
                    $this->property = '*';
                }
            }
