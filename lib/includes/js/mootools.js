...
this.Tips = new Class({
	...
	position: function(event){
		...
		for (var z in props){
			...
			//alert("props["+z+"] -> "+obj[props[z]] + ", tip=" + tip[z] + ", scroll=" + scroll[z] + ", size=" + size[z] + ", padding=" + this.options.windowPadding[z] + ", page=" + event.page[z] + ", offset=" + this.options.offset[z]);
			if (obj[props[z]] < 0)
			{
				bounds[z] = true;
				// make sure the tip stays within screen: bump the coordinate to zero:
				obj[props[z]] = 0;
			}
			if ((obj[props[z]] + tip[z] - scroll[z]) > size[z] - this.options.windowPadding[z]){
				obj[props[z]] = event.page[z] - this.options.offset[z] - tip[z];
				//alert("corrected props["+z+"] -> "+obj[props[z]] + ", tip=" + tip[z] + ", scroll=" + scroll[z] + ", size=" + size[z] + ", padding=" + this.options.windowPadding[z] + ", page=" + event.page[z] + ", offset=" + this.options.offset[z]);
				bounds[z+'2'] = true;
				if (obj[props[z]] < 0)
				{
					bounds[z] = true;
					// make sure the tip stays within screen: as a last resort, move the tip so it hungs the coordinate zero:
					obj[props[z]] = 0;
				}
			}
		}
