	...
	EditArea.prototype.maj_highlight= function(infos){
		...
		if(this.reload_highlight===true){
			...
			var base_step= 200;
			var cpt= 0;
			var end= Math.min(text_to_highlight.length, lastH.length);
            var step= base_step;
            // find how many chars are similar at the begin of the text						
			while(cpt<end && step>=1){
                if(lastH.substr(cpt, step) == text_to_highlight.substr(cpt, step)){
                    cpt+= step;
                }else{
		...
		var new_Obj= this.content_highlight.cloneNode(false);
		if( ( this.isIE && this.isIE < 8 ) || this.isOpera )
