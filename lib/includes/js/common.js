	...
	initializeWindows = function()
	{
		...
		$$('a.tabs').addEvent('click', function(e)
		{
			...
			*/
			...
			case "sys-perm":
				/* grid: 20x12.5 */
				dims = reduce_dimensions(20*40, 12.5*40);
				break;

			case "sys-pow":
				/* 24xY @ 8 users: 5 for the left side ==> ~2.5 per user. Y = 4+2+ ~12/18 per page */
				var ownercount = get_admin_user_count();
				var pagecount = get_total_page_count();
				var cx = 5 + 2.5 * ownercount;
				var cy = 4 + 2 + 12.0 * pagecount / 18.0;

				dims = reduce_dimensions(cx*40, cy*40);
				break;

			case "sys-tmp":
				/* TODO: adjust edit_area in template editor to fit the screen/window */
				/* now the template editor window is 13.5 high... */
				break;

			case "sys-usr":
				/* 22.5 x 13.5 ; height grows at about 4.8 blocks per 8 users + 4 hdr/ftr */
				var ownercount = get_admin_user_count();
				var cy = 2 + 2 + 4.8 * ownercount / 8;
				if (cy < 13.5) cy = 13.5;
