	...
	*/
	...
	function doEditor() {
		...
		MUI.myChain.chain(
			...
			function()
			{
				// superfluous, as the taskbar is already initialized by the MUI initializer itself...
				MUI.taskbar.initialize();
				MUI.myChain.callChain();
			},
			function()
			{
	...
	initializeWindows = function()
	{
		...
		MUI.clockWindow = function(){
			var cl = new MUI.Window({
				...
				//useCanvasControls: false, // (boolean) Set this to false if you wish to use images for the buttons.
				content: {
					url: '../lib/includes/js/plugins/clock.html', // '{plugins}coolclock/demo.html',
					require: {
						js: [
							'../lib/includes/js/plugins/coolclock.js', // '{plugins}coolclock/scripts/coolclock.js',
							'../lib/includes/js/plugins/moreskins.js'  // '{plugins}coolclock/scripts/moreskins.js'
						],
						onload: function(){
							if (CoolClock)
							{
								if (0) // diag / test only code
								{
									...
									for (var skin in CoolClock.config.skins)
									{
										...
										clocks.push('<div class="skintitle"><h3>'+skin+'</h3>');
