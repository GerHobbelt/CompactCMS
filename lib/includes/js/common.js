...
function lazyloading_commonJS_done(site_basedir)
{
	function editin_init() {
		...
		$$('.liverename').each(function(el) {
			...
			el.addEvent('click',function() {
				...
				$('notify_icon').removeClass('eraser');
		...
		var req = new Request.HTML({
			...
			onComplete: function() {
				...
				*/
				...
				if (menu_wrapper)
				{
					menu_wrapper.custom_slide('hide');
				}
				var form_wrapper = $('form_wrapper');
				if (form_wrapper)
				{
					form_wrapper.custom_slide('hide');
				}
	...
	*/
	...
	if ($('switchUserForm'))
	{
		$('switchUserForm').addEvent('submit', function(add) {
			new Event(add).stop();
	...
	function doEditor() {
		...
		MUI.myChain.chain(
			function(){
				...
				// quick hack/fix to work around buggy mochaUI (this is a temporary countermeasure)
				window.mochaUI_b0rked = false;
				try
				{
					MUI.initialize({
							path: {
				...
				{
					alert("internal error: mochaUI initialization b0rked.\n\n\nIt's one of those days again...\n\nMochaUI execution is corrupted.");
					window.mochaUI_b0rked = true;
					MUI.myChain.callChain();
				}
			},
			function()
			...
			function()
			{
				...
				*/
				if (!window.mochaUI_b0rked)
				{
					if (!MUI.desktop)
					{
						alert('internal error: MUI.desktop has not been initialized. MochaUI execution is corrupted.');
					}
					else
					{
				...
				{
					// dirty rotten hacks to make the admin screen look okay while mochaUI is down for the count...
					$('admin_index_page').setStyle('overflow', 'auto');
					$('pageWrapper').setStyle('overflow', 'visible');
				}
	...
	*/
	if (Browser.name != 'ie')
	{
		var fade_divs = $$('.may-fade');

		fade_divs.each(function(child) {
			//var siblings = fade_divs.erase(child);
			child.addEvents({
				mouseenter: function() {
					fade_divs.each(function(el, idx, arr) {
						if (el != child)
						{
							el.fade(0.5);
						}
						...
						{
							el.set('tween', {duration: 'short'}).fade(1);
						}
					});
				},
				mouseleave: function() {
	...
	$$('.toggle').addEvent('click', function(e) {
		e = new Event(e);
		e.stop();
		// re-use the existing slide Fx instance for this element, as it will track important hight/... values!
		$(this.rel).custom_slide('toggle');
	});
	...
	*/
	initializeWindows = function()
	{
		...
		if (!window.mochaUI_b0rked)
		{
			MUI.editWindow = function(id,url,title,dims)
			{
				...
				new MUI.Window({
					...
					onLoaded: function(){
						if (0) MUI.notification('Window content was loaded.');
					},
					onCloseComplete: function(){
						if (0) MUI.notification('The window is closed.');
					},
					onMinimize: function(){
						if (0) MUI.notification('Window was minimized.');
					},
					onMaximize: function(){
						if (0) MUI.notification('Window was maximized.');
					},
					onRestore: function(){
						if (0) MUI.notification('Window was restored.');
					},
					onResize: function(){
						//if (0) MUI.notification('Window was resized.');
					...
					onFocus: function(){
						if (0) MUI.notification('Window was focused.');
					},
					onBlur: function(){
						if (0) MUI.notification('Window lost focus.');
					},
					onDragStart: function(){
						if (0) MUI.notification('Window is being dragged.');
					},
					onDragComplete: function(){
						if (0) MUI.notification('Window drag complete.');
					}
				});
			};
		...
		{
			// in case mochaUI b0rked / is disabled:
		}

		$('clearCacheLink').addEvent('click', function(e){
			e.stop();
			//new Event(e).stop();
			...
			$('notify_icon').addClass('eraser');

			// Wait for response and act
			new Request.HTML({
				method: 'get',
				url: './includes/process.inc.php?action=clearcaches',
				update: notify,
				onComplete: function() {
					status.removeClass('loading');
				}
		...
		*/
		...
		var realResize = function(){
			/*
			See if the screen was resized indeed; if so, adjust the wrapper desktop <div> (if
			you don't set a height for that one, you won't have any scrollbar!) and when there's
			a change, we also go and adjust the MUI windows to the new state of affairs.
			*/
			var html_tag = $('admin_index_page');
			var dtcoords = html_tag.getCoordinates();
			var old_dtcoords = html_tag.retrieve('screen_coords', { width: 0, height: 0 });

			if (old_dtcoords.width == dtcoords.width && old_dtcoords.height == dtcoords.height)
				return;

			// adjust the desktop size:
			html_tag.setStyle('height', dtcoords.height);
			...
			// and lastly keep track of the screen info for next time...
			html_tag.store('screen_coords', dtcoords);
		};
