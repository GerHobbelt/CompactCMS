...
var FileManager = new Class({
	...
	options: {
		...
		directory: '',
		url: null,
		baseURL: '/',  // WARNING: this, and some of the other entries here, should EXACTLY mimic the values of their identically-named counterparts on the PHP side of things!
		assetBasePath: null,
		selectable: false,
		hideOnClick: false,
		language: 'en'
	...
	open: function(e){
		...
		this.fireEvent('complete', [
			this.normalize(this.options.baseURL + this.Directory + '/' + this.Current.retrieve('file').name),
		...
		$each(j.files, function(file){
			...
			if (file.mime!='text/directory')
				icons.push(new Asset.image(this.options.assetBasePath + 'disk.png', {title: this.language.download}).addClass('browser-icon').addEvent('click', (function(e){
					this.tips.hide();
					e.stop();
					window.open(this.normalize(this.options.baseURL + this.Directory + '/' + file.name));
	...
	fillInfo: function(file, path){
		...
		this.info.getElement('dd.filemanager-dir').empty().adopt(new Element('span', {text: this.options.baseURL + ' '}), text);
